...
简历写 Tool Runtime → 追问完整链路 → 你说 Schema 校验 → 继续追“Schema 能否保证安全” → 回答只能保证结构，业务权限仍由 Policy/Guard/HITL → 再追“错误后怎么办” → 回答结构化反馈、有限纠正和预算封顶。

### 追问树 2：MCP 分层

你说 MCP → 追问与 Function Calling 区别 → 回答模型意图 vs 外部能力协议 → 继续追 JSON-RPC → 回答消息格式层 → 再追“REST 不行吗” → 回答单 API 可直接封装，多来源发现和统一契约时 MCP 更合适。

### 追问树 3：Transport

简历写 stdio/Streamable HTTP → 追问怎么选 → 回答同机生命周期 vs 网络共享 → 继续追“Streamable HTTP 是 SSE 吗” → 回答不能等同旧单向 SSE transport → 再追连接成本 → 回答连接复用、超时和服务容量。

### 追问树 4：HITL 到强隔离

简历写 HITL → 追问能否保证安全 → 回答人也会误批 → 继续追审批绑定 → 回答 Tool/参数/调用/策略版本 → 再追 Prompt Injection → 回答不可信内容不能提升 Runtime 权限。

### 追问树 5：路径安全

你说 PathGuard → 追问 `../` → 回答规范化路径与工作区关系 → 继续追 symlink/TOCTOU → 回答应用层检查仍有竞态 → 再追真正方案 → 回答容器文件系统、低权限和受控挂载。

### 追问树 6：超时与幂等

你说 Tool timeout → 追问是否重试 → 回答 timeout 不等于未执行 → 继续追创建工单重复问题 → 回答 idempotency key/operation status → 再追对方不支持 → 回答人工确认或补偿，不盲重试。
